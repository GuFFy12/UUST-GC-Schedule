...
def format_event_as_string(event: Event):
    return f"{datetime.strftime(event.start, '%d.%m %H:%M')}, {event.summary}, {event.location}, {event.description}"
...
def main():
    ...
    for gc_event in gc_events:
        ...
        if is_event_updated:  # Если есть различия у занятия в расписании и календаре, обновляем событие.
            ...
            print(f"Занятие обновлено — {format_event_as_string(gc_event)} -> {format_event_as_string(schedule_event)}")

        schedule_events.pop(gc_event.start)  # Удаляем из списка расписания занятия, которые уже были в календаре.
